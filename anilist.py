...
class anilist:
    ...
    def query(qry: str, vars: dict):
        return requests.post(anilist.url, json={'query': qry, 'variables': vars}).json()
    
    @classmethod
    def squery_render(terms: str):
        media = anilist.simple_query(terms)['data']['Page']['media']
        responses = [
        ...
        for m in media:
            responses.append(
                InlineQueryResultArticle(
                    id=uuid4(),
                    title=(lambda t: f"{'[🌶] ' if m['isAdult'] else ''}{t['english'] or m['romaji']} ({m['native']})")(m['title']),
