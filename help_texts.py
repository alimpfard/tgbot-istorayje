    ...
    },
    {
        "list": [
            (
                ...
                + "Anywhere after the collection you can pass `!spoiler` (or `!spoil`/`!sp`) to send the media with a spoiler overlay.\n"
                + "  It is a flag, not a tag, so it never takes part in the search; to search for a literal tag, escape it as `\\!spoiler`.\n"
                + "  Note: Telegram's inline mode has no spoiler support, so this only takes effect when the bot sends the message itself (e.g. implicit group commands).\n"
                + "Examples:\n"
                ...
                + "    @istorayjebot gif spider !sp {$}   # search 'gif' for 'spider', default caption, spoilered\n"
